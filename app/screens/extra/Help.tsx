...
const Help: React.FC = () => {
  const { selected, handleContextChange, themes } = useContext(
    ThemeContext
  ) as ThemeContextType;
  ...
  return (
    <View style={[styles.container, { backgroundColor }]}>
      <View style={[styles.topBar, { backgroundColor: highlightColor }]}>
        <Text style={[styles.title, { color: textColor }]}>Ayuda y Soporte</Text>
      </View>
      <View style={styles.content}>
        <Text style={[styles.text, { color: textColor }]}>
          ¿Necesitas ayuda o soporte? Estamos aquí para ayudarte en todo momento.
        </Text>
        <Text style={[styles.text, { color: textColor }]}>
