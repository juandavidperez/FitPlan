...
// Tipar los parámetros que esperas recibir
type Params = {
  ejercicio: string; // Viene como string (serializado JSON)
};

const firstLetterToUpperCase = (string: string): string => {
...
const Exercise = () => {
  ...
  try {
    ejercicio = JSON.parse(params.ejercicio);
  } catch (error) {
    return (
      <View style={styles.container}>
        <Text style={{ color: "red" }}>Error al cargar el ejercicio.</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: highlightColor }]}>
      ...
      <View style={styles.details}>
        <View style={[styles.largeContainer, { backgroundColor }]}>
          <Text style={[styles.subtitle, { color: textColor }]}>
            <MaterialCommunityIcons name="arm-flex" size={24} color={textColor} />{" "}
            Músculos trabajados:{" "}
